...
m1_predict <- function(x) {
    ...
    # input is 1 word: will always find a match in bigrams table (maybe <UNK>)
    else if(length(tokens)==1) return(m1_bi[m1_bi$w1==tokens,]$w2)
    
    # else: input is 2+ words
    tokens <- tokens[(length(tokens)-1):length(tokens)]
    res <- m1_tri[m1_tri$w1==tokens[1] & m1_tri$w2==tokens[2],]$w3
    if(length(res)>0) return(res) # found exact match in trigrams
    return(m1_bi[m1_bi$w1==tokens[2],]$w2) # bigram prediction by last word
}
```