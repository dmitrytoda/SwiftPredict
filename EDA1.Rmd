...
twitterCorpus <- Corpus(VectorSource(twitter), readerControl = list(language = "UTF-8"))
blogsCorpus <- Corpus(VectorSource(blogs), readerControl = list(language = "UTF-8"))
newsCorpus <- Corpus(VectorSource(news), readerControl = list(language = "UTF-8"))
...
twitter1DF <- unigram(twitterCorpus)
blogs1DF <- unigram(blogsCorpus)
news1DF <- unigram(newsCorpus)
all1DF <- unigram(allCorpus)

c50 <- max(which(all1DF$cumpercentage<=.5))
c90 <- max(which(all1DF$cumpercentage<=.9))
c99 <- max(which(all1DF$cumpercentage<=.99))
```

Plot cumulative word percentages
```{r plotPercentageCovered}
ggplot(data=all1DF[as.integer(rownames(all1DF))%%5==1,], aes(x=seq_along(cumpercentage), y=cumpercentage)) + geom_point(color='steelblue') + scale_x_continuous(labels = comma) + labs(x='Words count', y='Cumulative % covered') + geom_hline(yintercept=.5) + geom_hline(yintercept=.9) + geom_hline(yintercept=.99)
```
